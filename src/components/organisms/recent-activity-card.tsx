...
const RecentActivityCard = ({
  ...
  const activityDuration = adventure.duration.split(':');
  const durationHours = activityDuration[0];
  const durationMinutes = activityDuration[1];
  const activityStartAt = activityDateTime.toISOString().slice(11, 16);
  const activityEndAt = addHours(activityDateTime, Number(durationHours));
  addMinutes(activityEndAt, Number(durationMinutes));
    ...
    >
      ...
      <div className="flex items-center justify-between w-full">
        ...
        >
          ...
          />
          {activityStartAt + ' - ' + activityEndAt.toISOString().slice(11, 16)}
        </MyTypography>
      </div>
