...
export default function InformacoesAtividade({
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (!selectedChildren) {
            toast.error(
                "Preencha se a atividade é com crianças ou sem crianças."
            );
            return;
        }
        ...
        const adventure = {
            ...
            priceChildren:
                priceChildren.length > 0
                    ? brlToApiNumberString(priceChildren)
                    : "0",
        ...
        try {
            ...
            if (err instanceof AxiosError) {
                ...
                toast.error(
                    "Erro desconhecido ao criar parceiro e/ou aventura."
                );
            }
        } finally {
            setIsLoading(false);
    ...
    const handleCreateAdventure = async (
        ...
        try {
            ...
            const adventure = {
                ...
                priceAdult:
                    priceAdult.length > 0
                        ? brlToApiNumberString(priceAdult)
                        : "0",
                priceChildren:
                    priceChildren.length > 0
                        ? brlToApiNumberString(priceChildren)
            ...
            const adventureResponse =
                await adventures.createAdventure(adventure);
            ...
            if (!isRepeatable && availableDates) {
                try {
                    await partnerService.createMoreSchedule(
                        adventureId,
                        availableDates
                    );
                } catch (err) {
    ...
    const [pricesAdult, setPricesAdult] = React.useState(
        calculateAdventurePrice(priceAdult)
    );
    const [pricesChildren, setPricesChildren] = React.useState(
        priceChildren ? calculateAdventurePrice(priceChildren) : null
    );

    useEffect(() => {
        setPricesAdult(calculateAdventurePrice(priceAdult));
        if (priceChildren) {
            setPricesChildren(calculateAdventurePrice(priceChildren));
        }
    ...
    return (
        <main className="w-full max-w-md md:max-w-3xl mx-auto md:p-4">
            <div
                className={cn("relative md:hidden", (edit || step) && "hidden")}
            >
                <MyLogo
                    variant="mobile"
                    width={100}
                ...
                {isInGroupTypes.map((item, index) => (
                    <MyButton
                        ...
                        onClick={() => {
                            ...
                            setAdventureData({
                                isInGroup:
                                    item.title == "Em grupo" ? true : false,
                            });
                        }}
                    >
                        <MyIcon
                ...
                {isChildrenAllowedTypes.map((item, index) => (
                    <MyButton
                        ...
                        onClick={() => {
                            ...
                            setAdventureData({
                                isChildrenAllowed:
                                    item.title == "Com crianças" ? true : false,
                            });
                        }}
                    >
                        <MyIcon
            ...
            <div className="space-y-4">
                <MyTextInput
                    ...
                    noHintText
                    leftIcon={
                        <MyIcon name="small-group" className="ml-2 mt-6" />
                    }
                    value={isInGroup ? personsLimit : "1"}
                    onChange={(e) =>
                        setAdventureData({
                            personsLimit: isInGroup
                ...
                {isChildrenAllowed && (
                    <MyTextInput
                        ...
                        noHintText
                        leftIcon={
                            <MyIcon name="dollar" className="ml-2 mt-6" />
                        }
                        value={priceChildren}
                        onChange={(e) =>
                            setAdventureData({
            ...
            <div className="flex flex-col my-8">
                ...
                <div className="flex justify-between mt-1">
                    ...
                    <MyTypography
                        ...
                        className="text-primary-600"
                    >
                        R$ {String(pricesAdult.totalCliente).replace('.', ',') ?? "0,00"}
                    </MyTypography>
                </div>
