...
import { MyCollapsible, CollapsibleContent } from "@/components/atoms/my-collapsible";
...
const VerticalStep = React.forwardRef<HTMLDivElement, VerticalStepProps>((props, ref) => {
  ...
  const renderChildren = () => {
    if (!expandVerticalSteps) {
      return (
        <MyCollapsible open={isCurrentStep}>
          <CollapsibleContent
            ref={(node) => {
              if (
    ...
    >
      ...
      <div
        className={cn(
          ...
          "absolute left-56 top-12 max-h-[23rem] overflow-y-auto w-[23rem]",
          // "absolute left-64 top-12",
        )}
      >
        {renderChildren()}
      </div>
