...
import { DialogContent, DialogHeader, DialogTitle, MyDialog } from "@/components/molecules/my-dialog";
  ...
  return (
    <MyDialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[90%] md:max-w-sm rounded-2xl py-20 px-6 text-center">
        ...
        <MyTypography variant="subtitle3" lightness={500} className="w-11/12 mx-auto">{descrition}</MyTypography>
        <MyButton variant="black-border" borderRadius="squared" size="lg"  className="mt-4 w-11/12 mx-auto font-bold" onClick={onClose}>
          {button}
        </MyButton>
      </DialogContent>
    </MyDialog>
